...
Names: Rahimatu Yushawu, Rayane Abdoul-Aziz and Jasmine N. Dezurn
Description : This project is a recipe book on popular African Dishes
...
def main():
    ...
    print("WELCOME! THIS PROGRAM IS DESIGNED TO HELP YOU WITH RECIPES ON AFRICAN POPULAR DISHES")
...
def manage(selected_recipe, recipe_book):
    while True:  # Allows user to try multiple times
        manage_recipe = input("Do you want to update or delete the recipe? Enter 'Update' or 'Delete'")
        ...
        if manage_recipe.lower() == "update":
            ...
            # This writes the recipe user feels should be updated to cooking instruction
            with open("new_recipe.txt", "w") as file:
                file.write(new_instructions)
            print("New instructions have been written to 'new_recipe.txt'.")

            break  # Exit the loop

        elif manage_recipe.lower() == "delete":
            del_instructions = input("Enter what to delete: ")
            recipe_book.delete_recipe(selected_recipe.name)

            # This shows the recipe user wants to delete from the instructions
            with open("Delete_Instruction.txt", "w") as file:
                file.write(del_instructions)
            print("New instructions have been written to 'Delete_Instruction.txt'.")
